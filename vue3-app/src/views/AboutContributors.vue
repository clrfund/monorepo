<template>
	<div class="about">
		...
		<p>
			You'll need some {{ chain.currency }} on {{ chain.label }} in order to submit transactions to the clr.fund
			smart contracts.
		...
		<ol>
			...
			<li>
				Make sure you have {{ nativeTokenSymbol }} in your BrightID-verified wallet on the
				{{ chain.label }} network
			</li>
			<li>
				<links to="/projects">Browse projects</links> and add some to your cart, adjusting your contributions to
				the amount of {{ nativeTokenSymbol }} you want.
			</li>
			<li>
		...
		<p>
			Note: you can only contribute once. After submitting your contributions, you can reallocate them as much as
			you would like before the end of the reallocation phase but you cannot add more funds to your total
			contribution amount.
		</p>
		<ol>
			<li>You can add and remove projects.</li>
			<li>
				Your reallocation total must be less than or equal to the original total (if it's less, the rest will go
				to the matching pool).
			</li>
		</ol>
		<h2>Contributing to the matching pool</h2>
		<p>
			Not sure which specific to contribute to? Contributions to the matching pool will be distributed to all
			projects that receive project-specific contributions during that round. The weighting of this distribution
		...
		<ol>
			<li>
				...
				{{ nativeTokenSymbol }} on {{ chain.label }}. {{ nativeTokenSymbol }} can be easily purchased on any
				major Ethereum-focused exchange.
			</li>
			<li>
