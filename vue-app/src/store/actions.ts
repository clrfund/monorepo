...
const actions = {
  ...
  async [LOAD_USER_INFO]({ commit, state }) {
    ...
    const isRegistered = userRegistryAddress
      ? await isVerifiedUser(userRegistryAddress, state.currentUser.walletAddress)
      : false

    if (nativeTokenAddress) {
      balance = await getTokenBalance(nativeTokenAddress, state.currentUser.walletAddress)
    }
  ...
  async [LOAD_CONTRIBUTOR_DATA]({ commit, state }) {
    ...
    const contributorKeypair = Keypair.createFromSeed(state.currentUser.encryptionKey)

    const stateIndex = await getContributorIndex(fundingRoundAddress, contributorKeypair.pubKey)
